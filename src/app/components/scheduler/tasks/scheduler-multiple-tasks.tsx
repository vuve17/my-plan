...
const UserMutipleTasks: React.FC<taskProps> = ({
  ...
  const [taskTooShortToDisplayAll, setTaskTooShortToDisplayAll] =
  ...
  function multipleTasksHeight() {
    if (startTime && endTime) {
      ...
      const diff = getDifferenceInHoursAndMinutes(
        new Date(startTime),
        new Date(endTime)
      );
  ...
  return (
    <Box
      ...
      onClick={(e) => setTaskModalMultipleActive(e)}
    >
      <Box
        sx={{
          display: "flex",
...
function openTaskModal(arg0: boolean): any {
