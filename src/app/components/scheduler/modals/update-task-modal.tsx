...
let newTaskSchema = Yup.object().shape({
  ...
  startDate: Yup.date()
    ...
    .test({
      ...
      exclusive: false,
      message:
        "Start Date must be the same as or after the current date and time",
...
const UpdateTaskModal: React.FC<UpdateTaskModalProps> = ({ task }) => {
  const handleStartTimeChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    formik.setFieldValue(
      "startDate",
      applyTimeToDate(formik.values.startDate, e.target.value)
    );
  };
  ...
  const handleEndTimeChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    formik.setFieldValue(
      "endDate",
      applyTimeToDate(formik.values.endDate, e.target.value)
    );
  };
  ...
  const taskModalDateString = useSelector(
    (state: RootState) => state.createTaskModal.taskModalDate
  );
  ...
  const initialBookmarkState = useSelector(
    (state: RootState) => state.bookmark.type
  );
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setErrors }) => {
      try {
        setDisableButtons(true);
        const { startTime, endTime, ...sentData } = values;
        ...
        if (response.ok) {
          ...
          dispatch(
            setSnackBarText(
              `Successfuly created ${sentData.title} ${sentData.taskType}`
            )
          );
            ...
            >
              ...
              >
                {formik.errors.startDate && formik.touched.startDate ? (
                  <ErrorBox text="Start Date must be the same as or after the current date and time" />
                ) : formik.errors.endDate && formik.touched.endDate ? (
                  <ErrorBox text="End Date must be the same or after Start Date" />
                ) : formik.errors.endTime && formik.touched.endTime ? (
                  <ErrorBox text={formik.errors.endTime} />
                ) : formik.errors.title && formik.touched.title ? (
                  <ErrorBox text={formik.errors.title} />
                ) : formik.errors.description && formik.touched.description ? (
                  <ErrorBox text={formik.errors.description} />
                ) : null}
              </Grid>
