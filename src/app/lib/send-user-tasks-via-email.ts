...
export async function sendEmail(): Promise<void> {
  ...
  try {
    ...
    `;
    const { rows: userTasks }: { rows: UserTask[] } = await client.query(query, [today]);

    const tasksByUser: { [userId: string]: { email: string; tasks: UserTask[] } } = userTasks.reduce(
      (acc, row) => {
    ...
    const emailPromises = Object.entries(tasksByUser).map(async ([userId, { email, tasks }]) => {
      ...
      const taskTableRows = tasks
        .map(
          ...
          <tr>
            ...
            <td style="border: 1px solid #ddd; padding: 8px;">${format(new Date(start_date), 'eee, dd. MMMM, HH:mm')}</td>
            <td style="border: 1px solid #ddd; padding: 8px;">${format(new Date(end_date), 'eee, dd. MMMM, HH:mm')}</td>
          </tr>
        `
        )
        .join('');
    ...
    console.log('Daily tasks processed and emails sent.');
  } catch (error) {
