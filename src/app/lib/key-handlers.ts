...
export async function createAccessKey(userId: string): Promise<createToken> {
    try {
        ...
        const insertKey = await insertKeyInDb(accessToken, userId, accessExpirationDate, "access")
...
export async function createRefreshKey(userId: string): Promise<createToken> {
    try {
        ...
        const refreshExpiration = Math.floor(Date.now() / 1000) + 2 * 30 * 24 * 60 * 60;
        const secret = new TextEncoder().encode(process.env.JWT_SECRET);
        ...
        const refreshExpirationDate = new Date(refreshExpiration * 1000).toISOString();

        const insertKey = await insertKeyInDb(refreshToken, userId, refreshExpirationDate, "refresh")
        if(!insertKey.success)
        {
...
export async function insertKeyInDb( key : string, userId: string, expiration_date: string, type: "access" | "refresh") {
...
export async function deleteAllKeys(userId: string, type: "refresh" | "access") {
    const client = await db.connect();
    try {
...
export async function readKeysFromDb (userId: string): Promise<KeyResult> {
    ...
    try{
        ...
        console.log("accessKey.rowCount : ", accessKeyResult.rowCount)
        console.log("refreshKey.rowCount : ", refreshKeyResult.rowCount)
        ...
        if(refreshKeyResult.rowCount > 1)
        {
            ...
            return { result: "many", message: "Too many refresh keys", userId : userId};
        }   

        if(accessKeyResult.rowCount > 1)
        {
            console.log("too many access keys")
            console.log(accessKeyResult.rows)
            // const accessKeysArray = accessKeyResult.rows.map(row => row.access_key);
            return { result: "many", message: "Too many access keys", userId: userId};
        }  
        ...
        if (refreshKey !== undefined && accessKey !== undefined) {
            ...
            return { result: "missing", message: "Refresh key is missing", accessKey: accessKey};
        } else if (accessKey === undefined && refreshKey) {
            console.log("access key doesn t  EXISTTT")
            return { result: "missing", message: "Access key is missing", refreshKey: refreshKey}
        } else {
        ...
        console.log(error);
        return { message: "something went wrong while reading keys" , result: "error" };
    }
    finally {
        if (client) {
            client.release(); 
...
export async function tokenExpirationDateValidation(token: string) {
    ...
    console.log("date token:" , tokenDate)
    if(tokenDate)
    {
        return true
    }
...
export async function handleReadKeysFromDb(userId:string): Promise <NextResponse <KeysInDb> > {
        const keysInDb: KeyResult = await readKeysFromDb(userId)
  
        if (keysInDb.result === "success" && keysInDb.refreshKey && keysInDb.accessKey) {

            const validRefreshKey = await tokenExpirationDateValidation(keysInDb.refreshKey) 
            const validAccessKey = await tokenExpirationDateValidation(keysInDb.accessKey) 

            if(validRefreshKey && validAccessKey)
            {
                console.log("success: ", keysInDb.refreshKey, " ", keysInDb.accessKey)
                return NextResponse.json({ refreshToken: keysInDb.refreshKey, accessToken: keysInDb.accessKey }, { status: 200 });    
            }
            ...
            else if(!validAccessKey && validRefreshKey)
            {
                ...
                {
                    return NextResponse.json({ message: "something went wrong with new access key" }, { status: 500 });
                }
                const newAccessKey = await createAccessKey(userId)
                if(!newAccessKey.success)
                {
                    return NextResponse.json({ message: "something went wrong with new access key" }, { status: 404 });    
                }
                return NextResponse.json({ refreshToken: keysInDb.refreshKey, accessToken: newAccessKey.token, }, { status: 200 });
            }
        } 
            ...
            if (keysInDb.message === "Access key is missing" && keysInDb.refreshKey) 
            {
                ...
                if(!validRefreshKey)
                {
                    const deleteOldRefreshKey = await deleteKey(keysInDb.refreshKey, "refresh")
                    const newRefreshKey: createToken = await createRefreshKey(userId)
                    if(!newRefreshKey.success || !deleteOldRefreshKey)
                    {
                        return NextResponse.json({ message: "Error creating new refresh key" }, { status: 500 });
                    }
                    const newAccessKey = await createAccessKey(userId)
                    if(!newAccessKey.success)
                    {
                        return NextResponse.json({ message: "Error creating new access key" }, { status: 500 });
                    }
                    return NextResponse.json({ refreshToken: newRefreshKey.token, accessToken: newAccessKey.token }, { status: 200 });
                }
                const newAccessKey = await createAccessKey(userId)
                if(!newAccessKey.success)
                {
                    return NextResponse.json({ message: "Create access key server error" }, { status: 500 });
                }
                return NextResponse.json({ refreshToken: keysInDb.refreshKey, accessToken: newAccessKey.token }, { status: 200 });
            } 

            else if (keysInDb.message === "Refresh key is missing" && keysInDb.accessKey) {
                const validAccessKey = await tokenExpirationDateValidation(keysInDb.accessKey) 
                if(!validAccessKey)
                {
                    const deleteOldAccessKey = await deleteKey(keysInDb.accessKey, "access")
                ...
                if(!newRefreshKey.success)
                {
                    return NextResponse.json({ message: "Create refresh key server error" }, { status: 500 });
                }
        ...
        else if (keysInDb.result === "many")
        {
            if(keysInDb.message === "Too many access keys" )
            {
                ...
                try{
                    ...
                    if(delAlltry.success)
                    {
                        ...
                        return NextResponse.json({ message: "something went wrong while deleting all access keys" }, { status: 500 })
                    }
                } 
                catch (error) 
                {
            ...
            else if(keysInDb.message === "Too many refresh keys" )
            {
                try {
                    ...
                    if (delAlltry.success) {
                        ...
                        return NextResponse.json({ message: "something went wrong while deleting all refresh keys" }, { status: 500 });
                    }
                } catch (error) {
                    return NextResponse.json({ message: "many else error" }, { status: 500 });
                }      
          ...
          return NextResponse.json({ message: "Read/Write error with user keys" }, { status: 500 })
        }
...
export async function Test(userId: string) {
    ...
    if (refreshKey !== undefined && accessKey !== undefined) {
        ...
        return { result: "success", message: "Keys exist in database", refreshToken: refreshKey, accessToken: accessKey }
    }
}
                ...
                // console.log("keysArray: ", keysInDb.keysArray)
                // for (const key of keysInDb.keysArray) {
                //     console.log("key: ", key)
                //     const verifiedKey = await tokenExpirationDateValidation(key)
                //     if(verifiedKey)
                //     { 
                //         continue
                //     }
                ...
                //     {
                //         console.log("delete")
                //         await deleteKey(key, "refresh");
                //     }
                // }
  
                // if(keysInDb.keysArray.length > 1)
                // {
                //     return NextResponse.json({ message: "still too many keys in db" }, { status: 500 });
                // }
                // else {
                //     await handleKeysReadFromDb(userId)
                // }  
                // else if(keysInDb.message === "Too many access keys")
                //     {
                //         console.log("keysArray: ", keysInDb.keysArray)
                //         for (const key of keysInDb.keysArray) {
                //         const verifiedKey = await tokenExpirationDateValidation(key)
